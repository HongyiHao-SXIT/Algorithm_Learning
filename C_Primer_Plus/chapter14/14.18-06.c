...
int read_datas(TEAM players[], int n, FILE *fp) {
  ...
  while (7 == fscanf(fp, "%d %18s %18s %d %d %d %d", &m, fname, lname,
                     &stage_num, &hit_num, &base_num, &rbi) &&
         !feof(fp) && count < n) {
    if (players[m].stage_num ==
        0)  // 球员信息为空则累计球员数量, 防止超出人数限制19人;
    {
      ++count;
    }
...
void show_messages(TEAM players[], int n) {
  ...
  printf(
      "Id   First_name   Last_name   Stage   Hit_rate   Base_num   RBI   "
      "Hit_rate\n");
  for (int i = 0; i < n; i++) {
    if (players[i].stage_num) {
      printf("%-4d %-12s %-10s %5d %7d %11d %8d %8.2f\n", players[i].id,
             players[i].fname, players[i].lname, players[i].stage_num,
             players[i].hit_num, players[i].base_num, players[i].rbi,
             players[i].hit_rate);
    }
  }
}

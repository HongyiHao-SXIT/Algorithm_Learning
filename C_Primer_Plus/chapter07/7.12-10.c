...
int main(void) {
  ...
  while (1) {
    ...
    switch (n) {
      case 1: {
        tax = (wage <= PLAN1 ? wage * RATE1
                             : PLAN1 * RATE1 + (wage - PLAN1) * RATE2);
        break;
      }
      case 2: {
        tax = (wage <= PLAN2 ? wage * RATE1
                             : PLAN2 * RATE1 + (wage - PLAN2) * RATE2);
        break;
      }
      case 3: {
        tax = (wage <= PLAN3 ? wage * RATE1
                             : PLAN3 * RATE1 + (wage - PLAN3) * RATE2);
        break;
      }
      case 4: {
        tax = (wage <= PLAN4 ? wage * RATE1
                             : PLAN4 * RATE1 + (wage - PLAN4) * RATE2);
        break;
      }
    }
    printf("Your tax: %g\n\n", tax);

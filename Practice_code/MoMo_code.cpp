...
private:
  ...
  void checkInactivity() {
    if (last_achievement_time.time_since_epoch().count() == 0)
      return;
...
public:
  void addQualification(ProfessionalLevel level) {
    checkInactivity();
    switch (level) {
    case ProfessionalLevel::Primary:
      reputation += 1;
      break;
  ...
  void addCompetition(CompetitionLevel level) {
    checkInactivity();
    switch (level) {
    case CompetitionLevel::City:
      reputation += 15;
      break;
    case CompetitionLevel::Province:
      reputation += 20;
      break;
  ...
  void addAchievement(AchievementType type) {
    checkInactivity();
    switch (type) {
    case AchievementType::InventionPatent:
      reputation += 20;
      break;
    case AchievementType::UtilityModelPatent:
      reputation += 30;
      break;
    case AchievementType::DesignPatent:
      reputation += 35;
      break;
    case AchievementType::SoftwareCopyright:
      reputation += 40;
      break;
    case AchievementType::OtherRegistration:
      reputation += 50;
      break;
    }
    last_achievement_time = std::chrono::system_clock::now();

...
int readStudents(Stu* stuHead) {
  ...
  for (int i = 0; i < countStudents; i++) {
    ...
    fscanf(fpReadStudents, "%s%s%d%d", readSavedStudents->name,
           readSavedStudents->idx, &readSavedStudents->grade,
           &readSavedStudents->numQue);
...
void saveProblems(Pro* proHead, int countProblems) {
  ...
  fprintf(fpWriteProblems, "Current number of questions: %d\n\n",
          countProblems);
  for (Pro* writeProblems = proHead->next; writeProblems != NULL;
       writeProblems = writeProblems->next) {
...
void saveStudents(Stu* stuHead, int countStudents) {
  ...
  fprintf(fpSaveStudents,
          "Total number of students who have answered questions: %d\n\n",
          countStudents);
...
void outputProblems(Pro* proHead, int countProblems) {
  ...
  title(" Question Bank ");
  endl;
  for (Pro* outProblems = proHead->next; outProblems != NULL;
       outProblems = outProblems->next) {
...
void outputStudents(Stu* stuHead, int countStudents) {
  ...
  for (Stu* point = stuHead->next; point != NULL; point = point->next) {
    printf("%s\t%s\t%d\t%d\n", point->name, point->idx, point->grade,
           point->numQue);
  }
  endl;
...
void deleteProblem(Pro* proHead, int toBeDeleted) {
  Pro* last = proHead;
  for (Pro* point = proHead->next; point != NULL;
       last = point, point = point->next) {
...
int startTeacher(Stu* stuHead, Pro* proHead, int countStudents,
                 int countProblems) {
  while (1) {
    cls;
    title(" Teacher Login ");
...
void startStudent(Stu* stuHead, Pro* proHead, int countStudents,
                  int countProblems) {
  ...
  addNewStudent->grade =
      (double)(countRightProblems * 100) / countSelectedProblems;
...
int main() {
  while (1) {
    ...
    switch (chooseTorS) {
      ...
      case 1:
        countProblems =
            startTeacher(stuHead, proHead, countStudents, countProblems);
        break;
      case 2:

...
void menu();
void floyd(Mgraph G, int D[MaxVertexNum][MaxVertexNum],
           int P[MaxVertexNum][MaxVertexNum]);
void opfloy(int n, int D[MaxVertexNum][MaxVertexNum],
            int P[MaxVertexNum][MaxVertexNum]);
...
void opfloy(int n, int D[MaxVertexNum][MaxVertexNum],
  ...
  printf(
      "Please enter your current location and the destination location (e.g., "
      "m,p):\n");
  scanf("%d,%d", &m, &p);
  i = m;
  j = p;
    ...
    for (w = 0; w < 5; w++) {
      ...
      for (t = 0; t < 5; t++) {
        ...
        for (r = 0; r < 5; r++) {
          ...
          sum = G->edges[m][q] + G->edges[q][w] + G->edges[w][t] +
                G->edges[t][r] + G->edges[r][m];
          end[m] = 1;
          end[q] = 1;
          end[w] = 1;
          end[t] = 1;
...
void CreateMGraph(Mgraph *G) {
  int i, j, k, w;
  printf(
      "Please enter the number of vertices and edges (input format: number of "

...
bool TreeIsEmpty(const Tree *ptree) { return NULL == ptree->root; }

bool TreeIsFull(const Tree *ptree) { return MAXITEMS == ptree->size; }
...
bool AddItem(const Item *pi, Tree *ptree) {
  ...
  if ((find = SeekItem(pi, ptree).child) != NULL) {
    ...
    if (temp !=
        NULL)  // 若temp为NULL则说明用户输入的宠物名字和种类与俱乐部中宠物相重复,故无法添加;
    {
      ...
      if ((node = (Kind *)malloc(sizeof(Kind))) == NULL) {
        ...
        node->next = NULL;
        for (temp = find->item.many_petkinds; temp->next != NULL;
             temp = temp->next) {
          continue;
        }
        temp->next = node;
...
bool DeleteItem(const Item *pi, Tree *ptree) {
  ...
  if (look.child->item.pets_count != 1)  // 结点中有多个同名不同类的宠物;
  {
    ...
    Kind *prior;
    Kind *current;
    for (prior = current = look.child->item.many_petkinds; current != NULL;
    ...
    if (0 == strcmp(look.child->item.many_petkinds->petkind,
                    pi->many_petkinds->petkind)) {
      if (look.parent == NULL) {
...
static int all_pets_numbers(const Trnode *root) {
  ...
  if (root != NULL) {
    ...
    count +=
        root->item.pets_count;  // 使用递归累加所有结点中的同名不同类的宠物数量;
